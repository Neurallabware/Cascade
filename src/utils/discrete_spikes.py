"""Infer discrete spikes from continuous spike rate predictions.

Copied from cascade2p/utils_discrete_spikes.py with updated imports.
All scientific logic is 100% identical to the original implementation.
...
def infer_discrete_spikes(
    spike_rates: NDArray[np.floating],
    model_name: str,
    model_folder: str = "Pretrained_models",
    verbosity: int = 1,
) -> tuple[NDArray[np.floating], list]:
    """Infer discrete spike times from continuous spike rate predictions.

    Parameters
    ----------
    spike_rates : numpy.ndarray
        Predicted spike rates with shape ``(nb_neurons, time_points)``.
    model_name : str
        Name of the model used for prediction.
    model_folder : str
        Path to the folder containing pretrained models.
    verbosity : int
        If 1, print progress messages.

    Returns
    -------
    tuple of (numpy.ndarray, list)
        - ``approximations_all``: Smoothed approximation of the spike rate.
        - ``spikes_all``: List of arrays with inferred spike locations for each neuron.
    """
    ...
    for neuron in range(spike_rates.shape[0]):
        ...
        offset = np.argmax(nnan_indices == True) - 1  # noqa: E712
        ...
        if np.sum(nnan_indices) > 0:
            ...
            support_slices = divide_and_conquer(
                prob_density, smoothing * sampling_rate
            )
...
def fill_up_APs(prob_density, smoothingX, nb_spikes, spike_locs):
    """Fill up action potential guesses using Monte Carlo / Metropolis sampling.

    Takes a probability distribution (prob_density) and an initial guess of
    spikes (spike_locs). The smoothed spikes generate an approximation of the
    probability. The difference between the probability distribution and the
    approximation is then compensated with additional spikes, sampled according
    to the distribution of the difference over time.
    """
...
def divide_and_conquer(prob_density, smoothingX):
    """Split probability density into continuous chunks of non-zero values.

    These are returned as "slices", i.e., ranges of indices (so-called "support").
    """
...
def systematic_exploration(prob_density, smoothingX, nb_spikes, spike_locs, approximation):
    """Systematically explore all positions for each spike.

    For each spike, all other possible locations in the probability density
    are tested. If any position is better than the initial guess, it is
    accepted; otherwise rejected.
    """
    # smoothed single spikes, initialized beforehand because creation takes time
    ...
    for spike_index, spike in enumerate(spike_locs):
        for timepoint in range(len(approximation)):
            ...
            error[timepoint] = np.sum(
                np.abs(prob_density - approximation_suggestion)
            )
...
def prune_APs(prob_density, smoothing, nb_spikes, spike_locs, approximation):
    """Remove spikes that worsen the approximation fit.

    Chooses each spike in turn and checks whether removing it improves the
    fit. If so, the spike is removed.
    """
    # produce approximation based on previously inferred spikes (spike_locs)
